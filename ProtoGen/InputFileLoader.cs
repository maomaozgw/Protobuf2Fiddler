            ...
            try
            {
                ...
                ProcessStartInfo psi = new ProcessStartInfo(
                    ...
                    string.Format(@"""--descriptor_set_out={0}"" ""--proto_path={1}"" ""--proto_path={2}"" --error_format=gcc ""{3}"" {4}",
                             tmp, // output file
                             Environment.CurrentDirectory.ToLower(), // primary search path
                             Path.GetDirectoryName(path).ToLower(), // secondary search path
                             Path.GetFullPath(path).ToLower(), // input file
